# -*- coding: utf-8 -*-
"""
Created on Tue Jul  5 21:03:24 2016

@author: minmhan
TODO: 
1. Add arbitrary number of hidden layers, which can be specified upon initialization.
2.  Alter training func so that output can be results from 1 to 5.
"""
...
class searchnet:
    ...
    def backpropagate(self,targets,N=0.5):
        ...
        for j in range(len(self.hiddenids)):
            error=0.0
            for k in range(len(self.urlids)):
                error=error+output_deltas[k]*self.wo[j][j]
    ...
    def trainquery(self,wordids,urlids,selectdurl):
        # generate a hidden node if necessary
        self.generatehiddennode(wordids,urlids)
        self.setupnetwork(wordids,urlids)
        self.feedforward()
        targets=[0.0]*len(urlids)
        targets[urlids.index(selectdurl)] = 1.0
...
print(mynet.trainquery([wWorld,wBank],[uWorldBank,uRiver,uEarth],uWorldBank))
print(mynet.getresult([wWorld,wBank],[uWorldBank,uRiver,uEarth]))            
